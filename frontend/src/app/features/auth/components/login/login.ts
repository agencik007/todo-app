    ...
    if (verified === 'success') {
      setTimeout(() => {
        this.messageService.add({
          ...
          detail: this.translate.instant('AUTH.LOGIN.SUCCESS.VERIFIED'),
          life: 5000,
        });
      }, 100);
    }
    ...
    this.authService.login(loginRequest).subscribe({
      next: () => {
        this.authService.getCurrentUser().subscribe({
          ...
          error: () => {
            this.error.set(
              this.translate.instant('AUTH.LOGIN.ERRORS.USER_DATA_FAILED'),
            );
            this.isLoading.set(false);
          },
        });
      ...
      error: (err: unknown) => {
        ...
        this.error.set(
          messageCode
            ...
            : this.translate.instant('AUTH.LOGIN.ERRORS.INVALID_CREDENTIALS'),
        );
        this.isLoading.set(false);
      },
    });
